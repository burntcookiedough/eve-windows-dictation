...
from voiceserver.protocol.frames import (
    ClosingFrame,
    ClosingReason,
    ErrorFrame,
    FinalTextFrame,
...
class FrameSender:
    ...
    async def send_error(self, code: ErrorCode, message: str) -> None:
        ...
        await self._ws.send_json(frame.model_dump())
        logger.warning("[%s] Sent error frame: %s - %s", self._session_id, code, message)
    ...
    async def send_partial(self, text: str, confidence: float) -> None:
        ...
        await self._ws.send_json(frame.model_dump())
        logger.debug("[%s] Sent partial: %r (conf=%.2f)", self._session_id, text[:50], confidence)
    ...
    async def send_final(self, text: str, confidence: float) -> None:
        ...
        await self._ws.send_json(frame.model_dump())
        logger.info("[%s] Sent final: %r (conf=%.2f)", self._session_id, text[:100], confidence)
