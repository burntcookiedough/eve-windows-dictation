...
def test_legacy_environment_override_is_migrated_before_settings_validation(
    ...
    assert settings.nemotron_model == "nvidia/nemotron-speech-streaming-en-0.6b"
    assert settings.nemotron_device == "auto"
    assert json.loads(settings_file.read_text(encoding="utf-8"))["whisper_model"] == "small"


def test_persisted_whisper_settings_are_not_rewritten_or_semantically_changed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    settings_file = tmp_path / "server-settings.json"
    raw = {
    ...
    assert json.loads(settings_file.read_text(encoding="utf-8")) == raw
